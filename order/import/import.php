...
//                    $this->item_data['order_number'] = $this->wt_parse_order_number_field($value);
    ...
    //            if ('order_key' == $column) {
    //                $this->item_data['order_key'] = $this->wt_parse_int_field($value);
    //                continue;
    //            }
    ...
    public function wt_parse_status_field($value) {
        ...
        if (!empty($value)) {
            ...
            if ($found_status) {
                ...
                throw new Exception(sprintf('Skipped. Unknown order status %s (%s).',$value));    
            }
        }
...
//            $set_props = $object->set_props(array_diff_key($data, array_flip(array('meta_data', 'raw_image_id', 'raw_gallery_image_ids', 'raw_attributes'))));
...
//                foreach ($data['order_items'] as $line_item) {                    
//                    $product = get_product($line_item['product_id']);
//                    $order->add_product($product, $line_item['qty']); //(get_product with id and next is for quantity)
//                }
//               
//            }
...
//            $order->set_address($data['billing'], 'billing');
//            $order->set_address($data['shipping'], 'shipping');
//            $object->add_coupon('Fresher', '10', '2'); // accepted param $couponcode, $couponamount,$coupon_tax
...
//            $order->set_cart_hash( $data['cart_hash'] );
